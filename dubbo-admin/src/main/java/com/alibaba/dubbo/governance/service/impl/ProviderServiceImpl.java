...
public class ProviderServiceImpl extends AbstractService implements ProviderService {
    ...
    @java.lang.Override
    public Map<String, String> findServicesDescMap() {
        Map<String, String> ret = new HashMap<String, String>();
        ConcurrentMap<String, Map<Long, URL>> providerUrls = getRegistryCache().get(Constants.PROVIDERS_CATEGORY);
        if (providerUrls != null) {
            for (String s : providerUrls.keySet()) {
                Map<Long, URL> longURLMap = providerUrls.get(s);
                StringBuilder sb = new StringBuilder();

                Set<String> descs = new HashSet<String>();
                if (longURLMap != null) {
                    for (Long o : longURLMap.keySet()) {
                        URL url = longURLMap.get(o);
                        if (url != null) {
                            String javaDesc = url.getParameter("javaDesc");
                            if (javaDesc != null && javaDesc.length() > 0) {
                                descs.add(javaDesc);
                            }
                        }
                    }
